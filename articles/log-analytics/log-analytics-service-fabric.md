...
I denne artikel beskrives det, hvordan du bruger løsningen Service-strukturen i Log Analytics til at identificere og foretage fejlfinding af problemer på tværs af din Service-strukturen klynge, ved at få indsigt i hvordan din tjeneste strukturen noder klarer sig, og hvordan dine programmer og micro-tjenester der kører.

Tjenesten strukturen løsningen bruger Azure diagnosticering data fra din Service-strukturen FOS, ved at indsamle disse data fra Azure WAD tabellerne. Log Analytics læser derefter tjenesten strukturen framework hændelser, herunder **Pålidelige tjenestehændelser**, **Agent begivenheder**, **Funktionsdygtige begivenheder**og **brugerdefineret ETW begivenheder**. Dashboardet Service-strukturen løsning viser væsentlige problemer og relevante begivenheder i dit miljø, Service-strukturen.

## <a name="installing-and-configuring-the-solution"></a>Installation og konfiguration af løsningen

Følg disse tre nemme trin for at installere og konfigurere løsningen:

1. Sikre, at arbejdsområdet OMS, du bruger er knyttet til det samme Azure abonnement, du brugte til at oprette alle de klyngeressourcer, herunder lagerplads konti. Se [Introduktion til Log Analytics](log-analytics-get-started.md) oplysninger om oprettelse af et OMS arbejdsområde.
2. Konfigurere OMS for at indsamle og få vist Service-strukturen logfiler.
3. Aktivere løsningen Service-strukturen i arbejdsområdet.

## <a name="configure-oms-to-collect-and-view-service-fabric-logs"></a>Konfigurere OMS for at indsamle og få vist Service-strukturen logfiler
I dette afsnit lærer du at konfigurere OMS for at hente Service-strukturen logfiler. Loggene kan du få vist, analysere og foretage fejlfinding af problemer i din klynge eller i de programmer og tjenester, der kører i pågældende klynge ved hjælp af portalen OMS.

>[AZURE.NOTE] Filtypenavnet Azure Diagnostics skal konfigureres til at overføre logfiler til lagerplads tabeller, der svarer til hvad OMS søger efter. Se, [hvordan du indsamle logge med Azure diagnosticering](../service-fabric/service-fabric-diagnostics-how-to-setup-wad.md) kan finde flere oplysninger om, hvordan du indsamle logge. Konfiguration af indstillinger for eksemplerne i denne artikel viser du hvilke navnene på lagerplads tabeller skal være. Når diagnosticering er konfigureret på klyngen og overføres logfiler til en lagerplads konto, er næste trin at konfigurere OMS for at indsamle disse logfiler.

Sørg for, at du opdaterer afsnittet **EtwEventSourceProviderConfiguration** i filen **template.json** til at føje poster til den nye EventSources, før du anvender konfiguration opdateringen ved at køre **deploy.ps1**. Tabellen til at uploade er den samme som (ETWEventTable). I øjeblikket læse OMS kun ETW programhændelser fra den pågældende tabel. Dog understøttelse af brugerdefinerede ETW tabeller er under udvikling.

Følgende værktøjer bruges til at udføre nogle af handlingerne i dette afsnit:

-   Azure PowerShell
-   [Handlinger Management pakke](http://www.microsoft.com/oms)

### <a name="configure-an-oms-workspace-to-show-the-cluster-logs"></a>Konfigurere en OMS arbejdsområde for at få vist loggene klynge

Når du har oprettet et OMS arbejdsområde som beskrevet ovenfor, er næste trin at konfigurere arbejdsområde for at adskille loggene fra tabellerne Azure-lager, hvor de bliver overført fra klyngen ved filtypenavnet Diagnostics. Gøre dette, skal du køre følgende PowerShell-script:
...
Når du har konfigureret arbejdsområdet OMS læse fra Azure tabellerne i kontoen lagerplads, log på portalen Azure, og vælg OMS arbejdsområdet i **Alle de ressourcer**. Når du har valgt, skal du se antallet af lagerplads logfiler, der er knyttet til dette OMS arbejdsområde-konto. Vælg feltet **lagerplads konto logge** , og Bekræft på listen over lagerplads konto logfiler at kontoen lagerplads har forbindelse til dette OMS arbejdsområde:

![Lagerplads konto logfiler](./media/log-analytics-service-fabric/sf1.png)

## <a name="enable-the-service-fabric-solution"></a>Aktivere løsningen Service-strukturen
Brug følgende script til at føje løsningen til arbejdsområdet OMS. Køre scriptet i PowerShell, ved hjælp af det Azure abonnement, der er knyttet til arbejdsområdet OMS, du vil aktivere løsningen Service-strukturen i.
...
>[AZURE.NOTE] Du kan ændre omfanget af disse begivenheder i tjenesten strukturen løsningen ved at klikke på **Data ud fra de seneste 7 dage** øverst på dashboardet. Du kan også vise hændelser, der opstår i de seneste 7 dage, 1 dag eller 6 timer. Eller du kan vælge **brugerdefineret** til at angive et brugerdefineret datointerval.


## <a name="troubleshoot-your-service-fabric-and-oms-configuration"></a>Fejlfinding i forbindelse med din tjeneste-strukturen og OMS konfiguration

Hvis du har brug at bekræfte din OMS konfiguration, fordi du kan ikke få vist begivenhed data i OMS, kan du bruge nedenstående script. Den læser din tjeneste strukturen diagnosticering konfiguration, kontrollerer, om data, der skrives til tabeller, og det bekræfter, at OMS er konfigureret til at læse fra tabellerne.
