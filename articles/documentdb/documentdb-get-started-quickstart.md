...
Velkommen til NoSQL selvstudiet til Azure DocumentDB .NET SDK! Når du få hurtig start projektet eller gennemførelse selvstudiet, har du en console-program, der opretter og forespørgsler DocumentDB ressourcer.

- **[Hurtig start](#quickstart)**: hente eksempelprojektet, tilføje forbindelsesoplysningerne og har en DocumentDB-app, der kører på mindre end 10 minutter.
- **[Selvstudium](#tutorial)**: oprette Hurtig start app fra bunden på 30 minutter.
...
## <a name="quickstart"></a>Hurtig start

1. Hent eksempel project .zip fra [GitHub](https://github.com/Azure-Samples/documentdb-dotnet-getting-started-quickstart/archive/master.zip) eller klone [documentdb-dotnet-få-gang-Hurtig start](https://github.com/Azure-Samples/documentdb-dotnet-getting-started-quickstart) repo.
2. Bruge Azure-portalen til at [oprette en DocumentDB-konto](documentdb-create-account.md).
3. Erstat EndpointUri og primær nøgle-værdier i App.config-filen med de værdier, der er hentet fra [Azure-portalen](https://portal.azure.com/), ved at navigere til **DocumentDB (NoSQL)** blade, og derefter klikke på **kontonavnet**og derefter klikke på **taster** i menuen ressource.
    ![Skærmbillede af værdien EndpointUri og primær nøgle for at erstatte i App.config](./media/documentdb-get-started-quickstart/nosql-tutorial-documentdb-keys.png)
4. Oprette projektet. Vinduet console viser de nye ressourcer blive oprettet, forespørges, og derefter ryddes op.
    
    ![Skærmbillede af output fra konsollen](./media/documentdb-get-started-quickstart/nosql-tutorial-documentdb-console-output.png)

## <a id="tutorial"></a>Selvstudium

Dette selvstudium vejleder dig gennem oprettelse af en DocumentDB database, en DocumentDB af websteder og JSON dokumenter. Du kan derefter forespørgsel samlingen, og Ryd op og slette databasen. Dette selvstudium opbygger den samme projekt som projektet, Hurtig start, men du skal oprette den trinvist og modtager beskrivelse af den kode, du føjer til projektet.
...
Før du tilføjer koden for oprettelse af en database, kan du tilføje en hjælpeprogram metode til at skrive til konsollen.
...
Kopiér og Indsæt følgende kode i din **GetStartedDemo** metode under oprette databasen. Dette vil oprette en navngivet *FamilyCollection_va*dokument af websteder.
