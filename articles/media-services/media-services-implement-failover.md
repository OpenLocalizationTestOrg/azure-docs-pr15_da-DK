<properties 
    pageTitle="Implementere failover streaming scenarie | Microsoft Azure" 
    description="Dette emne viser, hvordan du implementere failover streaming scenario." 
...
#<a name="implementing-failover-streaming-scenario"></a>Implementere failover streaming scenarie

Denne gennemgang viser hvordan du kopierer indhold (BLOB) fra et aktiv i en anden for at kunne håndtere redundans til streaming efter behov. Dette scenario er nyttig til kunder, der vil konfigurere deres LEVERANDØR(er) Sådan flytter du mellem to datacentre tale om en afbrydelse i en af vores datacentre.
Denne gennemgang bruger Microsoft Azure Media Services SDK, Microsoft Azure Media Services REST-API og Azure lagerplads SDK til at vise følgende opgaver.

1. Konfigurere en Media Services-konto i "Data Center A".
1. Overføre en mezzanine-fil til en kilde aktiv.
1. Kod aktivet i flere bit rente MP4-filer. 
1. Oprette en skrivebeskyttet SAS locator for aktivet kilde til har læseadgang til objektbeholderen i kontoen lagerplads, der er knyttet til kilde aktiv.
1. Få objektbeholder navn på kilde aktiv fra den skrivebeskyttet SAS locator, der er oprettet i ovenstående trin. Vi har brug for disse oplysninger til kopiering BLOB mellem lagerplads konti (forklaret senere i emnet).
1. Oprette en origin locator for aktivet, der blev oprettet af opgaven kodning. 

Derefter og til at håndtere sekundære:

1. Konfigurere en Media Services-konto i "Data Center B".
1. Oprette et destinationsprogram tomme aktiv i mållejerens Media Services-konto.
1. Oprette en Skriv SAS locator for target tomme aktivet have skriveadgang til objektbeholderen i mållejerens lagerplads konto, som er knyttet til mål aktiv.
1. Bruge Azure lagerplads SDK til at kopiere BLOB (aktiv filer) mellem kilde lagerplads konto i "Data Center A" og mål lagerplads konto i "Data Center B" (disse lagerplads konti er knyttet til aktiverne interesse.)
1. Knytte BLOB (aktiv filer), der er blevet kopieret til objektbeholderen target blob med target aktiv. 
1. Oprette en origin locator for aktivet i "Data Center B", og Angiv locator-Id, der blev oprettet for aktivet i "Data Center A". 
1. Det giver dig streaming URL-adresserne hvor de relative stier på URL-adresserne er den samme (kun for grundtallet URL-adresser er forskellige). 
 
Derefter kan du oprette et CDN oven på disse origin locators for at håndtere afbrydelser. 

Overvej følgende gælder:

- Den aktuelle version af Media Services SDK understøtter ikke oprettelse af en locator med et bestemt locator-id. For at opnå denne opgave skal bruge vi Media Services REST-API.
- Den aktuelle version af Media Services SDK understøtter ikke automatisk generere IAssetFile oplysninger, der skal tilknyttes en aktiv aktiv filer. For at opnå denne opgave skal bruge vi CreateFileInfos Media Services REST-API. 
- Lagerplads krypteret Aktiver (AssetCreationOptions.StorageEncrypted) understøttes ikke for gentagelse (da krypteringsnøglen vil være forskellige i begge Media Services-konti). 
- Hvis du vil benytte de dynamiske emballagen, skal du først få mindst én efter behov Streaming reserveret enheder. Du kan finde yderligere oplysninger finder [Dynamisk emballagen Aktiver](media-services-dynamic-packaging-overview.md).
 

>[AZURE.NOTE]Overvej at bruge Media Services [Af Microsoft værktøj](http://replicator.codeplex.com/) som et alternativ til at implementere et failover streaming scenarie manuelt. Dette værktøj kan du gentage Aktiver på tværs af to Media Services-konti.

##<a name="prerequisites"></a>Forudsætninger
 
- To Media Services konti i et nyt eller eksisterende Azure abonnement. Se, [hvordan du opretter en Media Services-konto](media-services-portal-create-account.md).
- Operativsystemer: Windows 7, Windows 2008 R2 eller Windows 8.
- .NET framework 4.5 eller .NET Framework 4.
- Visual Studio 2010 SP1 eller nyere (professionel, Premium, Ultimate eller Express).
...
1. Brug Visual Studio til at oprette en ny løsning, der indeholder projektet C# Console-program. Angiv HandleRedundancyForOnDemandStreaming for navnet, og klik derefter på OK.
1. Oprette mappen SupportFiles på det samme niveau som HandleRedundancyForOnDemandStreaming.csproj projektfilen. Oprette mapperne OutputFiles og MP4Files under mappen SupportFiles. Kopiere en .mp4-fil i mappen MP4Files (i dette eksempel bruges filen BigBuckBunny.mp4). 
...
1. Tilføje andre referencer, der kræves til dette projekt: system.Configuration i stor, afsnitsgruppen og sektionsgruppen.
...
##<a name="add-code-that-handles-redundancy-for-on-demand-streaming"></a>Tilføje kode, der håndterer redundans til streaming efter behov.



1. Tilføj følgende klasse niveau felter til klassen Program.
...
1. Erstatte standard Main metode definitionen med et følgende:
...
1. Metodedefinitioner, der kaldes fra hovedvinduet er defineret nedenfor.
...
Nu kan du bruge en trafik leder skal distribuere anmodninger mellem de to datacentre og dermed failover i forbindelse med afbrydelser.
