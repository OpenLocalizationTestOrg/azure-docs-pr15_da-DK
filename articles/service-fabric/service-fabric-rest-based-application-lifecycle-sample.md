<properties
   pageTitle="REST-baseret program livscyklus eksempel | Microsoft Azure"
   description="En Microsoft Azure Service strukturen eksempel, der viser programmet livscyklus ved hjælp af brugergrænsefladen i tjenesten strukturen RESTEN."
...
# <a name="rest-based-application-lifecycle-sample"></a>REST-baseret program livscyklus eksempel

Dette eksempel demonstrerer Service-strukturen programmet livscyklus gennem REST-API-opkald. Du kan finde flere oplysninger om tjenesten strukturen programmet livscyklus, [Service-strukturen programmet livscyklus](service-fabric-application-lifecycle.md).

Dette eksempel udfører følgende:

* Bestemmelser eksemplet **WordCount 1.0.0** fra programpakke WordCount i billedet store.
* Viser en liste over programmet posttyper, som omfatter WordCount 1.0.0.
* Opretter WordCount program som **strukturen: / WordCount**.
* Viser listen over programmer, som omfatter strukturen: / WordCount version 1.0.0.
* Bestemmelser på 1.1.0 version WordCount stikprøvens fra programpakke **WordCountUpgrade** i store billede.
* Viser en liste over programmet posttyper, som omfatter både WordCount 1.0.0 og **WordCount 1.1.0**.
* Opgraderer programmet WordCount til version 1.1.0.
* Viser listen over programmer, som omfatter WordCount version 1.1.0, men omfatter ikke længere WordCount version 1.0.0.
* Sletter programmet WordCount.
* Viser listen over programmer, som ikke længere omfatter strukturen: / WordCount.
* Unprovisions 1.1.0 version af WordCount eksempel.
* Viser en liste over programmet posttyper, som omfatter WordCount 1.0.0, men omfatter ikke længere WordCount 1.1.0.
* Unprovisions 1.0.0 version af WordCount eksempel.
* Viser en liste over programmet posttyper, som ikke længere omfatter WordCount.


## <a name="prerequisites"></a>Forudsætninger

Dette eksempel bruger [WordCount eksempel](http://aka.ms/servicefabricsamples) (findes i de **Første gang** prøver). Eksemplet WordCount skal der oprettes først, og klik derefter to programpakker skal kopieres til store på billede.

|Mappe|Beskrivelse|
|------|-----------|
|WordCount|Programmet WordCount eksempel. Filen **ApplicationManifest.xml** indeholder **ApplicationTypeVersion = "1.0.0"**.|
|WordCountUpgrade|Programmet WordCount eksempel. Filen ApplicationManifest.xml skal ændres til **ApplicationTypeVersion = "1.1.0"** tillade programmet opgraderingen skal udføres.|

For at oprette programpakker og kopiere dem til store billede skal du benytte følgende fremgangsmåde:

1. Kopiere **C:\ServiceFabricSamples\Services\WordCount\WordCount\pkg\Debug** til **C:\Temp\WordCount**. Dette opretter programpakke WordCount.
2. Kopiere C:\Temp\WordCount til **C:\Temp\WordCountUpgrade**. Dette opretter **WordCountUpgrade** programpakke.
3. Åbn **C:\Temp\WordCountUpgrade\ApplicationManifest.xml** i et tekstredigeringsprogram.
4. Ændre egenskaben **ApplicationTypeVersion** til **"1.1.0"**i elementet **ApplicationManifest** .  Dette opdaterer versionsnummeret af programmet.
5. Gem den ændrede ApplicationManifest.xml-fil.
6. Som administrator for at kopiere programmerne til store på billedet, skal du køre følgende PowerShell-script:
...
Når du er færdig med PowerShell-script, er dette program er klar til at køre.

## <a name="example"></a>Eksempel

I følgende eksempel vises Service-strukturen programmet livscyklus.
